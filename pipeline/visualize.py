...
def make_document_heatmap(
    text,
    sentences_words_index,
    sentences_words_sentiment,
    sentences_cluster,
    save=None,
    polarity=1,
):
    ...
    for i in range(n_sentences):
        ...
        sentence_text = sentence_text[: num_chars - 1]
        ...
        sentiment_values = np.array(
            [
                value.item() if type(value) != int else value
    ...
    f, axes = plt.subplots(
        n_sentences,
        ncols=1,
        gridspec_kw={"height_ratios": sentences_height},
        figsize=(cell_width * n_limit, sum(sentences_height) * num_rows),
    )
    ...
    for i in range(n_sentences):
        ...
        for j in range(len(words_index)):
            sentiment_values[
                (words_index[j][0] - sentence_offset) : (
                    words_index[j][1] - sentence_offset
                )
            ] = [words_sentiment[j]] * (words_index[j][1] - words_index[j][0])
        ...
        average_sentiment = round(
            sum([i for i in sentiment_values if i != 0])
            / len([i for i in sentiment_values if i != 0]),
            2,
        )
        ...
        axes[i].set_title(
            "Sentence {sentence_index} : cluster {cluster_index} & average sentiment {average_sentiment}".format(
                sentence_index=i,
...
def make_heatmaps(df, n, output_dir):
    ...
    for index, row in df_sample.iterrows():
        hmap = make_document_heatmap(
            row["text"],
            row["sentences_index"],
            row["sentences_sentiment"],
            row["sentences_cluster"],
            output_dir + "/heatmap_{}".format(row["review_id"]),
        )
...
def make_global_analysis(df, df_clusters, output_dir):
    ...
    for index, row in df.iterrows():
        ...
        for i in range(n_sentences):
            ...
            if len(words_index) > 0:
                sentence_text = text[words_index[0][0] : words_index[-1][1]]
                sentence_offset = words_index[0][0]
                # We need to have a value per character
                sentiment_values = [0] * len(sentence_text)
                for j in range(len(words_index)):
                ...
                clusters_sentiment[cluster] = clusters_sentiment.get(cluster, []) + [
                    average_sentiment
                ]
    ...
    radar = Radar(
        fig,
        [cluster if cluster is not None else "No Cluster" for cluster in clusters_list],
        labels,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--processed-file",
        type=str,
        default="data/output/input_example_output.pkl",
    ...
    parser.add_argument(
        "--clusters-file",
        type=str,
        default="data/output/input_example_clusters.pkl",
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/visualization/input_example",
    ...
    parser.add_argument(
        "--heatmap-n", type=int, default=1, help="How many heatmap to create"
    )
    ...
    if not (args.heatmap):
        make_heatmaps(df, 2, args.output_dir)

    if not (args.global_analysis):
